define([
  ...
  "knockout"
      ...
      function request_render_module(parentModulePromise, moduleID, args, element, bindingContext) {
        parentModulePromise = !parentModulePromise ? Promise.resolve() : parentModulePromise;
        return parentModulePromise.then(function(parentModule){
          debugger
          console.log("requesting module:", moduleID, "args:", args);
          var modulePromise = load_module(moduleID, args);
          return render_child_module(modulePromise, element, bindingContext).then(function(module){
            debugger
            console.log("module", moduleID, "rendered.");
            return module;
          });
        });
      }

      function update_module(moduleID, args, element, bindingContext) { //notify after module is inserted in element
        return request_render_module(null, moduleID, args, element, bindingContext).then(function(module){
      ...
      KO.bindingHandlers.include_module = {
        ...
        update: function(element, valueAccessor, allBindings, viewmodel, bindingContext){
          debugger
          //see https://stackoverflow.com/questions/19422801/knockoutjs-bindinghandler-with-childbindingcontext-data-parent
          var moduleID = KO.unwrap(valueAccessor()); //see app_module in /spa/template.html
          var args = allBindings.get("module_args");
          //moduleURL = "/app_modules/" + moduleURL + "/loader.js";
          update_module(moduleID, args, element, bindingContext);
        }
      };
